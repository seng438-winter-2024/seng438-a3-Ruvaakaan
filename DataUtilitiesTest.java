...
public class DataUtilitiesTest extends DataUtilities {
	...
	public void testCreateNumberArray2DEmptyData() {
		DataUtilities.createNumberArray2D(new double[2][1]); // creates an empty 2 dimensional array with 1 item each to
																// be used
	}
...
//        	System.out.println(result.getValue(i).doubleValue());
	...
	public void testNullInputCalculateRowTotal() {
		...
		} catch (Exception e) { // If a Exception other than InvalidParameterException is caught then the test
								// will fail since we are expecting a InvalidParameterException
		}
	}
	...
	public void testCalculateColumnTotalValidDataTableWithNonNumericValues() {
		try {
			...
			// Catch any exception other than InvalidParameterException which will cause the
			// test to fail
		}
	}
    ...
    public void calculateRowTotalEmptyTableWithInvalidColumns() {
        ...
        //Invocation and assertion
        assertEquals(7, DataUtilities.calculateRowTotal(RowInvalidColsValues, 1, new int[]{0}), .0001);
    }
